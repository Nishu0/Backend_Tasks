...
app.get("/notes/create",checkAuthenticated,async(req,res)=>{
...
app.get('/allNotes',checkAuthenticated,async(req,res)=>{
	try{
		const allnotes=await Note.find({user:req.user._id}).exec()
		res.send(allnotes)
	}
	catch(err){
		console.log(err)
...
app.get('/allBookmarks',checkAuthenticated,async(req,res)=>{
	try{
		const allbookmarks=await Bookmark.find({user:req.user._id}).exec()
